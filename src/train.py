...
def main(args: CliArgs):
    ...
    if args.model_type == "classification":
        ...
        if args.data_ratio != 1:
            train_loader, test_loader, class_names = create_partial_data_loaders(train_dir = "dataset/recycleable-and-non-recycleable-dataset/TRAIN",
                            test_dir = "dataset/recycleable-and-non-recycleable-dataset/TEST", batch_size=args.batch_size, transforms=transform, ratio = args.data_ratio)
        else:
            train_loader, test_loader, class_names = create_data_loaders(train_dir = "dataset/recycleable-and-non-recycleable-dataset/TRAIN",
                            test_dir = "dataset/recycleable-and-non-recycleable-dataset/TEST", batch_size=args.batch_size, transforms=transform)
        if args.model == "TinyVGG":
            model = TinyVGG(input_shape=(3,224,224), hidden_units = 10, output_shape=len(class_names))
        elif args.model == "ResNet50":
            model = create_resnet_50_model(num_classes = len(class_names))
        ...
        # Train the model
        train(model, train_loader, test_loader, optimizer, loss_fn, device = device, epochs=args.epochs, writer = writer)
        model_name = (f"{args.model}_model1.pth - {args.epochs} epochs, {args.learning_rate} lr, {args.batch_size} batch size, {args.data_ratio} data ratio")
        torch.save(model.state_dict(), f"{model_name}")
        print(f"Model saved to {model_name}")
        writer.close()
